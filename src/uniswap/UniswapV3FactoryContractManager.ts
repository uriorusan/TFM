...
export class UniswapV3FactoryContractManager extends ContractManager<IUniswapV3Factory> {
    ...
    async getPool(tokenA: string, tokenB: string, poolFee: number) {
    ...
    async getUsdcWethPool(): Promise<string> {
        let usdcAddress = AaveV3Ethereum.ASSETS.USDC.UNDERLYING;
        let poolAddress = await this.getPool(usdcAddress, this.wEthAddress, 3000);
        return poolAddress;
    }

    async getDaiWethPool(): Promise<string> {
        let daiAddress = AaveV3Ethereum.ASSETS.DAI.UNDERLYING;
        let poolAddress = await this.getPool(daiAddress, this.wEthAddress, 3000);
        return poolAddress;
    }

    async getDaiUsdcPool(): Promise<string> {
        let daiAddress = AaveV3Ethereum.ASSETS.DAI.UNDERLYING;
        let usdcAddress = AaveV3Ethereum.ASSETS.USDC.UNDERLYING;
        let poolAddress = await this.getPool(daiAddress, usdcAddress, 3000);
        return poolAddress;
    }
}
