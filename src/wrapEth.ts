...
export async function wrapEth(amountEthToSwap: string, destinationAddress: string) {
    console.log(`Will wrap ${amountEthToSwap} ETH to WETH and send it to ${destinationAddress}`)
    ...
    console.log(`Withdrawing ${amountEthToSwap} WETH from the wrapped ETH gateway AAVE Pool and sending it to ${destinationAddress}`);
